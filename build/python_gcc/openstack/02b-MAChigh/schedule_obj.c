...
This was done on 2017-04-24 16:58:26.968736.
*/
...
//=== from otf
uint8_t schedule_getNumOfSlotsByType(OpenMote* self, cellType_t type){
   uint8_t returnVal;
   scheduleEntry_t* scheduleWalker;
...
void schedule_advanceSlot(OpenMote* self) {
   ...
   INTERRUPT_DECLARATION();
   DISABLE_INTERRUPTS();
   if ((self->schedule_vars).currentScheduleEntry->slotOffset >= ((scheduleEntry_t*)(self->schedule_vars).currentScheduleEntry->next)->slotOffset
       ) {
       // one slotframe has elapsed
 sf0_notifyNewSlotframe(self);
   }   
