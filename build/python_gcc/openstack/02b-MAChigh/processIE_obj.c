...
This was done on 2017-04-24 16:58:11.968073.
*/
...
port_INLINE void processIE_prepend_sixtopIE(OpenMote* self, 
      OpenQueueEntry_t* pkt, 
      uint8_t           len
   ){
   payload_IE_ht payload_IE_desc;
...
port_INLINE uint8_t processIE_prependSlotframeLinkIE(OpenMote* self, OpenQueueEntry_t* pkt){
 ...
 packetfunctions_reserveHeaderSize(self, pkt,5);
   pkt->payload[0] = schedule_getFrameNumber(self);  
   pkt->payload[1] = schedule_getFrameHandle(self);
...
port_INLINE uint8_t processIE_prependTSCHTimeslotIE(OpenMote* self, OpenQueueEntry_t* pkt){
   uint8_t    len;
   mlme_IE_ht mlme_subHeader;
   ...
   len = 0;
   duration = ieee154e_getSlotDuration(self);
...
port_INLINE uint8_t processIE_prependChannelHoppingIE(OpenMote* self, OpenQueueEntry_t* pkt){
   uint8_t    len;
   mlme_IE_ht mlme_subHeader;
...
port_INLINE uint8_t processIE_prepend_sixSubIEHeader(OpenMote* self, 
    OpenQueueEntry_t*    pkt,
    uint8_t len
  ){
    mlme_IE_ht mlme_subHeader;
...
port_INLINE uint8_t processIE_prepend_sixGeneralMessage(OpenMote* self, 
    OpenQueueEntry_t*    pkt,
    uint8_t code
    ){
    uint8_t    len = 0;
...
port_INLINE uint8_t processIE_prepend_sixSubID(OpenMote* self, OpenQueueEntry_t*    pkt){
    uint8_t    len = 0;
...
port_INLINE uint8_t processIE_prepend_sixCelllist(OpenMote* self, 
    OpenQueueEntry_t*    pkt,
...
port_INLINE void processIE_retrieveSlotframeLinkIE(OpenMote* self, 
 ...
 schedule_setFrameNumber(self, numSlotFrames);
 ...
 schedule_setFrameHandle(self, sfInfo.slotframehandle);
 ...
 schedule_setFrameLength(self, sfInfo.slotframesize);
...
port_INLINE void processIE_retrieve_sixCelllist(OpenMote* self, 
    OpenQueueEntry_t*   pkt,
    uint8_t             ptr,
    uint8_t             length,
    cellInfo_ht*        cellList
