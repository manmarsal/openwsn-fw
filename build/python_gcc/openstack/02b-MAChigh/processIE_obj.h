...
This was done on 2017-04-24 16:53:19.417108.
*/
...
void processIE_prepend_sixtopIE(OpenMote* self, 
   OpenQueueEntry_t*    pkt, 
   uint8_t              len
);
//===== prepend IEs
...
uint8_t processIE_prependTSCHTimeslotIE(OpenMote* self, 
    OpenQueueEntry_t*    pkt
);
uint8_t processIE_prependChannelHoppingIE(OpenMote* self, 
    OpenQueueEntry_t*    pkt
);
uint8_t processIE_prepend_sixSubIEHeader(OpenMote* self, 
    OpenQueueEntry_t*    pkt,
    uint8_t len
);
uint8_t processIE_prepend_sixGeneralMessage(OpenMote* self, 
    OpenQueueEntry_t*    pkt,
    uint8_t code
);
uint8_t processIE_prepend_sixSubID(OpenMote* self, 
    OpenQueueEntry_t*    pkt
);
uint8_t processIE_prepend_sixCelllist(OpenMote* self, 
    OpenQueueEntry_t*    pkt,
...
void processIE_retrieve_sixCelllist(OpenMote* self, 
    OpenQueueEntry_t*   pkt,
    uint8_t             ptr,
    uint8_t             length,
    cellInfo_ht*        cellList
