...
This was done on 2017-04-24 16:56:48.569423.
*/
/**
...
m_securityLevelDescriptor* IEEE802154_security_securityLevelDescriptorLookup(OpenMote* self, uint8_t frameType,
                                                                             uint8_t cfi);

m_deviceDescriptor* IEEE802154_security_deviceDescriptorLookup(OpenMote* self, open_addr_t*     address,
                                                               open_addr_t*     PANID,
                                                               m_keyDescriptor* keyDescriptor);

m_keyDescriptor* IEEE802154_security_keyDescriptorLookup(OpenMote* self, uint8_t      KeyIdMode,
...
void IEEE802154_security_init(OpenMote* self) {
   ...
   memset(&(self->ieee802154_security_vars).Key_1[0], 0, 16);
   (self->ieee802154_security_vars).Key_1[0] = 0x36;
   (self->ieee802154_security_vars).Key_1[1] = 0x54;
   (self->ieee802154_security_vars).Key_1[2] = 0x69;
   (self->ieee802154_security_vars).Key_1[3] = 0x53;
   (self->ieee802154_security_vars).Key_1[4] = 0x43;
   (self->ieee802154_security_vars).Key_1[5] = 0x48;
   (self->ieee802154_security_vars).Key_1[6] = 0x20;
   (self->ieee802154_security_vars).Key_1[7] = 0x6d;
   (self->ieee802154_security_vars).Key_1[8] = 0x69;
   (self->ieee802154_security_vars).Key_1[9] = 0x6e;
   (self->ieee802154_security_vars).Key_1[10] = 0x69;
   (self->ieee802154_security_vars).Key_1[11] = 0x6d;
   (self->ieee802154_security_vars).Key_1[12] = 0x61;
   (self->ieee802154_security_vars).Key_1[13] = 0x6c;
   (self->ieee802154_security_vars).Key_1[14] = 0x31;
   (self->ieee802154_security_vars).Key_1[15] = 0x35;

   //Initialization of the MAC Security Level Table
   for (i = 0; i < IEEE154_TYPE_UNDEFINED; i++) { // iterate through all frame types
      (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].FrameType = i;
      (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].CommandFrameIdentifier = 0;
      (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].DeviceOverrideSecurityMinimum = FALSE;
      //list of allowed security levels
      if (i == IEEE154_TYPE_BEACON){   // beacons can only be authenticated but not encrypted
         (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].AllowedSecurityLevels[0] = IEEE154_ASH_SLF_TYPE_MIC_32;
         (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].AllowedSecurityLevels[1] = IEEE154_ASH_SLF_TYPE_MIC_64;
         (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].AllowedSecurityLevels[2] = IEEE154_ASH_SLF_TYPE_MIC_128;
      } else { // all other frame types are encrypted + authenticated
         (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].AllowedSecurityLevels[0] = IEEE154_ASH_SLF_TYPE_ENC_MIC_32;
         (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].AllowedSecurityLevels[1] = IEEE154_ASH_SLF_TYPE_ENC_MIC_64;
         (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].AllowedSecurityLevels[2] = IEEE154_ASH_SLF_TYPE_ENC_MIC_128;
      }
      (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].SecurityMinimum = IEEE154_ASH_SLF_TYPE_ENC_MIC_32;
   }

   //Initialization of MAC KEY TABLE
   memset(&(self->ieee802154_security_vars).MacKeyTable,
          0,
          sizeof(m_macKeyTable));

   //Initialization of MAC DEVICE TABLE
   memset(&(self->ieee802154_security_vars).MacDeviceTable,
          0,
          sizeof(m_macDeviceTable));

   //Initialization of Frame Counter
   (self->ieee802154_security_vars).m_macFrameCounterMode = 0x05; // For TSCH, we use implicit 5 byte Frame Counter (ASN) 

   //macDefaultKeySource - shared
   (self->ieee802154_security_vars).m_macDefaultKeySource.type = ADDR_64B;
   (self->ieee802154_security_vars).m_macDefaultKeySource.addr_64b[0] = 0x14;
   (self->ieee802154_security_vars).m_macDefaultKeySource.addr_64b[1] = 0x15;
   (self->ieee802154_security_vars).m_macDefaultKeySource.addr_64b[2] = 0x92;
   (self->ieee802154_security_vars).m_macDefaultKeySource.addr_64b[3] = 0x00;
   (self->ieee802154_security_vars).m_macDefaultKeySource.addr_64b[4] = 0x00;
   (self->ieee802154_security_vars).m_macDefaultKeySource.addr_64b[5] = 0x15;
   (self->ieee802154_security_vars).m_macDefaultKeySource.addr_64b[6] = 0x16;
   (self->ieee802154_security_vars).m_macDefaultKeySource.addr_64b[7] = 0x5a;

   //store the key and related attributes
   //Creation of the KeyDescriptor
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[0].KeyIdLookupList.KeyIdMode = IEEE154_ASH_KEYIDMODE_DEFAULTKEYSOURCE;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[0].KeyIdLookupList.KeyIndex = IEEE802154_SECURITY_K1_KEY_INDEX;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[0].KeyIdLookupList.KeySource.type = ADDR_64B;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[0].KeyIdLookupList.KeySource = (self->ieee802154_security_vars).m_macDefaultKeySource;

   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[0].KeyIdLookupList.Address = (self->ieee802154_security_vars).m_macDefaultKeySource;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[0].KeyIdLookupList.PANId = *( idmanager_getMyID(self, ADDR_PANID));
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[0].KeyUsageList[0].FrameType = IEEE154_TYPE_BEACON;

   memcpy(&(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[0].key[0],
          &(self->ieee802154_security_vars).Key_1[0],
          16);

   (self->ieee802154_security_vars).MacDeviceTable.DeviceDescriptorEntry[0].deviceAddress = (self->ieee802154_security_vars).m_macDefaultKeySource;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[0].DeviceTable = &(self->ieee802154_security_vars).MacDeviceTable;

   //KEY 2: 'deadbeeffacecafedeadbeeffacecafe'
   memset(&(self->ieee802154_security_vars).Key_2[0], 0, 16);
   (self->ieee802154_security_vars).Key_2[0] = 0xde;
   (self->ieee802154_security_vars).Key_2[1] = 0xad;
   (self->ieee802154_security_vars).Key_2[2] = 0xbe;
   (self->ieee802154_security_vars).Key_2[3] = 0xef;
   (self->ieee802154_security_vars).Key_2[4] = 0xfa;
   (self->ieee802154_security_vars).Key_2[5] = 0xce;
   (self->ieee802154_security_vars).Key_2[6] = 0xca;
   (self->ieee802154_security_vars).Key_2[7] = 0xfe;
   (self->ieee802154_security_vars).Key_2[8] = 0xde;
   (self->ieee802154_security_vars).Key_2[9] = 0xad;
   (self->ieee802154_security_vars).Key_2[10] = 0xbe;
   (self->ieee802154_security_vars).Key_2[11] = 0xef;
   (self->ieee802154_security_vars).Key_2[12] = 0xfa;
   (self->ieee802154_security_vars).Key_2[13] = 0xce;
   (self->ieee802154_security_vars).Key_2[14] = 0xca;
   (self->ieee802154_security_vars).Key_2[15] = 0xfe;
   ...
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[1].KeyIdLookupList.KeyIdMode = IEEE154_ASH_KEYIDMODE_DEFAULTKEYSOURCE;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[1].KeyIdLookupList.KeyIndex = IEEE802154_SECURITY_K2_KEY_INDEX;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[1].KeyIdLookupList.KeySource.type = ADDR_64B;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[1].KeyIdLookupList.KeySource = (self->ieee802154_security_vars).m_macDefaultKeySource;

   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[1].KeyIdLookupList.Address = (self->ieee802154_security_vars).m_macDefaultKeySource;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[1].KeyIdLookupList.PANId = *( idmanager_getMyID(self, ADDR_PANID));
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[1].KeyUsageList[0].FrameType = IEEE154_TYPE_DATA;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[1].KeyUsageList[1].FrameType = IEEE154_TYPE_ACK;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[1].KeyUsageList[2].FrameType = IEEE154_TYPE_CMD;

   memcpy(&(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[1].key[0],
          &(self->ieee802154_security_vars).Key_2[0],
          16);

   (self->ieee802154_security_vars).MacDeviceTable.DeviceDescriptorEntry[1].deviceAddress = (self->ieee802154_security_vars).m_macDefaultKeySource;
   (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[1].DeviceTable = &(self->ieee802154_security_vars).MacDeviceTable;

   (self->ieee802154_security_vars).minimal = IEEE802154_SECURITY_MINIMAL_PROC;
}
...
void IEEE802154_security_prependAuxiliarySecurityHeader(OpenMote* self, OpenQueueEntry_t* msg){
   ...
   auxiliaryLength = IEEE802154_security_auxLengthChecking(msg->l2_keyIdMode,
                                                           ...
                                                           (self->ieee802154_security_vars).m_macFrameCounterMode); //length of Key ID field
 ...
 packetfunctions_reserveHeaderSize(self, msg, sizeof(uint8_t));
   ...
   switch (msg->l2_keyIdMode){
      case IEEE154_ASH_KEYIDMODE_IMPLICIT: //no KeyIDMode field - implicit
    	 temp_keySource = &(self->ieee802154_security_vars).m_macDefaultKeySource;
         memcpy(&(msg->l2_keySource), temp_keySource, sizeof(open_addr_t));
         break;
      case IEEE154_ASH_KEYIDMODE_DEFAULTKEYSOURCE:// macDefaultKeySource
         msg->l2_keySource = (self->ieee802154_security_vars).m_macDefaultKeySource;
         break;
 ...
 openserial_printError(self, COMPONENT_SECURITY,ERR_SECURITY,
   ...
   if (frameCounterSuppression == IEEE154_ASH_FRAMECOUNTER_PRESENT){
      ...
      // reserve space
 packetfunctions_reserveHeaderSize(self, msg,sizeof(macFrameCounter_t));
 ...
 packetfunctions_reserveHeaderSize(self, msg, sizeof(uint8_t));
   ...
   if ((self->ieee802154_security_vars).m_macFrameCounterMode == 0x04) { // 4 byte Frame Counter
...
owerror_t IEEE802154_security_outgoingFrameSecurity(OpenMote* self, OpenQueueEntry_t*   msg){
   ...
   if ((self->ieee802154_security_vars).minimal == 0) {
      //the frame counter is carried in the frame, otherwise 1;
      frameCounterSuppression = IEEE154_ASH_FRAMECOUNTER_SUPPRESSED;

      //search for a key
      keyDescriptor = IEEE802154_security_keyDescriptorLookup(self, msg->l2_keyIdMode,
                                                              &msg->l2_keySource,
                                                              msg->l2_keyIndex,
                                                              &msg->l2_keySource,
                                                              ( idmanager_getMyID(self, ADDR_PANID)),
                                                              msg->l2_frameType);
 ...
 ieee154e_getAsn(self, vectASN);//gets asn from mac layer.
      ...
      key = msg->l2_frameType == IEEE154_TYPE_BEACON ? (self->ieee802154_security_vars).Key_1 : (self->ieee802154_security_vars).Key_2;
   }
 ...
 openserial_printError(self, COMPONENT_SECURITY,ERR_SECURITY,
   ...
   switch (msg->l2_keyIdMode){
      case IEEE154_ASH_KEYIDMODE_IMPLICIT:
         ...
         temp_addr = &(self->ieee802154_security_vars).m_macDefaultKeySource;
...
owerror_t IEEE802154_security_incomingFrame(OpenMote* self, OpenQueueEntry_t* msg){
 ...
 openserial_printError(self, COMPONENT_SECURITY,ERR_SECURITY,
      ...
      deviceDescriptor = IEEE802154_security_deviceDescriptorLookup(self, &msg->l2_keySource,
 idmanager_getMyID(self, ADDR_PANID),
                                                                   keyDescriptor);
 ...
 openserial_printError(self, COMPONENT_SECURITY,ERR_SECURITY,
      ...
      securityLevelDescriptor = IEEE802154_security_securityLevelDescriptorLookup(self, msg->l2_frameType,
...
m_securityLevelDescriptor* IEEE802154_security_securityLevelDescriptorLookup(OpenMote* self, uint8_t frameType,
   ...
   DISABLE_INTERRUPTS();
   for (i=0; i<4; i++){
      if ((self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].FrameType != IEEE154_TYPE_CMD
         && frameType == (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].FrameType){

         ENABLE_INTERRUPTS();
         return &(self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i];
      }

      if ((self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].FrameType == IEEE154_TYPE_CMD
         && frameType == (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].FrameType
         && cfi == (self->ieee802154_security_vars).MacSecurityLevelTable.SecurityDescriptorEntry[i].CommandFrameIdentifier){
...
m_deviceDescriptor* IEEE802154_security_deviceDescriptorLookup(OpenMote* self, open_addr_t* Address,
   ...
   for (i=0; i<MAXNUMNEIGHBORS; i++){
      if (( packetfunctions_sameAddress(self, Address,
         &keyDescriptor->DeviceTable->DeviceDescriptorEntry[i].deviceAddress)== TRUE)
         &&( packetfunctions_sameAddress(self, PANId,
         &(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.PANId))
         ){
            ENABLE_INTERRUPTS();
...
m_keyDescriptor* IEEE802154_security_keyDescriptorLookup(OpenMote* self, uint8_t      KeyIdMode,
   ...
   if (KeyIdMode == IEEE154_ASH_KEYIDMODE_IMPLICIT){
      for (i=0; i<MAXNUMKEYS; i++ ){
         if ((self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.Address.type == ADDR_64B){
            if ( packetfunctions_sameAddress(self, DeviceAddress,&(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.Address)
                                           && packetfunctions_sameAddress(self, panID,
                                           &(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.PANId)){
               ENABLE_INTERRUPTS();
               return &(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i];
             }
         }
      }
   }
   ...
   if (KeyIdMode == IEEE154_ASH_KEYIDMODE_DEFAULTKEYSOURCE){
      for (i=0; i<MAXNUMKEYS; i++ ){
         if (KeyIndex == (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.KeyIndex
             && packetfunctions_sameAddress(self, keySource,&(self->ieee802154_security_vars).m_macDefaultKeySource)){
            ENABLE_INTERRUPTS();
            return &(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i];
         }
      }
   ...
   if (KeyIdMode == IEEE154_ASH_KEYIDMODE_EXPLICIT_16){
      for (i=0; i<MAXNUMKEYS; i++ ){
         if (KeyIndex == (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.KeyIndex){
            if (keySource->addr_16b[0] == (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.KeySource.addr_16b[0] &&
               keySource->addr_16b[1] == (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.KeySource.addr_16b[1]
               && packetfunctions_sameAddress(self, panID, &(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.PANId)
               ){
               ENABLE_INTERRUPTS();
               return &(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i];
            }
   ...
   if (KeyIdMode == IEEE154_ASH_KEYIDMODE_EXPLICIT_64){
      for (i=0; i<MAXNUMKEYS; i++ ){
         if (KeyIndex == (self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.KeyIndex){
            if ( packetfunctions_sameAddress(self, keySource,&(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.KeySource)
                && packetfunctions_sameAddress(self, panID, &(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.PANId)
               ){
               ENABLE_INTERRUPTS();
               return &(self->ieee802154_security_vars).MacKeyTable.KeyDescriptorElement[i];
